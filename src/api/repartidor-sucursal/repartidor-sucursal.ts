...
export const getAllRepartidorSucursales = async (): Promise<RepartidorSucursal[]> => {
  const response = await apiClient.get<{ success: boolean; data: RepartidorSucursal[] }>("/rep-sucu");
...
export const getRepartidorSucursal = async (
  ...
  const response = await apiClient.get<{ success: boolean; data: RepartidorSucursal }>(`/rep-sucu/${user_id}/${sucursal_id}`);
  return response.data.data;
};
...
export const createRepartidorSucursal = async (
  ...
  const response = await apiClient.post<{ success: boolean; data: RepartidorSucursal }>("/rep-sucu", payload);
  return response.data.data;
};
...
export const deleteRepartidorSucursal = async (
  ...
  const response = await apiClient.delete<{ success: boolean; data: { message: string } }>(`/rep-sucu/${user_id}/${sucursal_id}`);
  return response.data.data;
};
